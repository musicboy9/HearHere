...
func MyGetOpenALAudioData(inFileURL: NSURL, _ outDataSize: inout ALsizei, _ outDataFormat: inout ALenum, _ outSampleRate: inout ALsizei) -> UnsafeMutableRawPointer {
    ...
    return UnsafeMutableRawPointer(theData!)
}

func MyFreeOpenALAudioData(data: UnsafeMutableRawPointer, _ dataSize: ALsizei) {
    //let theData = UnsafeMutablePointer<CChar>(data)
    let theData = UnsafeMutablePointer<CChar>.allocate(capacity: Int(dataSize))
    //theData.assign(from: data, count: Int(dataSize))
    theData.deallocate(capacity: Int(dataSize))
}
